...
namespace JW.Base.Lang {
    ...
    public class ExcuteCodeHelper {
        ...
        /// 可执行字符串数组   
        /// </param>   
        public ExcuteCodeHelper(ExcuteCode[] items) {
        ...
        /// <param name="name">执行字符串名称</param>   
        public ExcuteCodeHelper(Type returnType, string expression, string name) {
            //创建可执行字符串数组   
            ExcuteCode[] items = { new ExcuteCode(returnType, expression, name) };
            ConstructEvaluator(items);      //调用解析字符串构造函数进行解析   
        }
        ...
        public ExcuteCodeHelper(ExcuteCode item) {
            ExcuteCode[] items = { item };//将可执行字符串项转为可执行字符串项数组   
            ConstructEvaluator(items);      //调用解析字符串构造函数进行解析   
        }
        ...
        private void ConstructEvaluator(ExcuteCode[] items) {
            //创建C#编译器实例
            ...
            foreach (ExcuteCode item in items)               //遍历每一个可执行字符串项   
            {
    ...
    public class ExcuteCode {
        ...
        /// <param name="expression">执行表达式</param>   
        /// <param name="name">执行字符串名称</param>   
        public ExcuteCode(Type returnType, string expression, string name) {
